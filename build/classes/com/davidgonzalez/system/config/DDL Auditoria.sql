...
create table Users(
    ...
    user varchar(25) not null check(length(user)<=50),
    password varchar(35) not null check(length(password)<=35),
    id_user varchar(36) not null, # es con uuid()
...
delimiter $$
    ...
    begin 
        ...
        values(name_p, lastname_p, email_p, user_p, 
				password_p, uuid());
    end $$
delimiter ;

call sp_create_users("da", "da", "da@", "da", "da123");